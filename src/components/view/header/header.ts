...
class Header extends DomElement {
  ...
  constructor() {
    super();
    this.cart = this.createElement('a', 'header__cart position-relative', { href: '/cart' });
    ...
    this.cartTotal = this.createElement('div', 'header__cart-total-price');
  }
  ...
  public draw(data: ProductsData, cart: CartItem[]): void {
    ...
    this.cart.appendChild(this.cartItemsCount);
