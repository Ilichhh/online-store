...
class FiltersBlock extends DomElement {
  ...
  public draw(data: ProductsData, params: QueryParams): HTMLElement {
    ...
    // Draw buttons
    const buttonsWrapper: HTMLElement = this.createElement('div', 'filters-block__buttons row mb-3');
    const buttonColRes: HTMLElement = this.createElement('div', 'col');
    const buttonGridRes: HTMLElement = this.createElement('div', 'd-grid gap-2');
    const buttonColCopy: HTMLElement = this.createElement('div', 'col');
    const buttonGridCopy: HTMLElement = this.createElement('div', 'd-grid gap-2');
    ...
    // Draw Price
    const priceWrapper: HTMLElement = this.createElement('div', 'filters-block__price mb-4');
    const priceHeader: HTMLElement = this.createElement('h5', 'filters-block__price-header mb-5', {}, 'Price');
    ...
    // Draw Category
    const categoryWrapper: HTMLElement = this.createElement('div', 'filters-block__category mb-4');
    const categoryHeader: HTMLElement = this.createElement('h5', 'filters-block__category-header', {}, 'Category');
    ...
    // Draw Brand
    const brandWrapper: HTMLElement = this.createElement('div', 'filters-block__brand mb-4');
    const brandHeader: HTMLElement = this.createElement('h5', 'filters-block__brand-header', {}, 'Brand');
    ...
    // Draw Stock
    const stockWrapper: HTMLElement = this.createElement('div', 'filters-block__stock mb-4');
    const stockHeader: HTMLElement = this.createElement('h5', 'filters-block__stock-header mb-5', {}, 'Stock');
  ...
  public drawCheckboxFilter(
    ...
    let allFiltersArr: string[] = data.products.map((e) => e[filter]);
    allFiltersArr = [...new Set(allFiltersArr)];
    const checkedArr: string[] = params[filter]?.split('%');
    const filteredData: Product[] = this.filter.filterData(data, params);

    allFiltersArr.forEach((item) => {
      const wrapper: HTMLElement = this.createElement('div', 'checkbox-filter d-flex form-check');
      const input: HTMLInputElement = <HTMLInputElement>(
        this.createElement('input', 'checkbox-filter__cb form-check-input', { type: 'checkbox', value: item, id: item })
      );
      const label: HTMLElement = this.createElement('label', 'form-check-label', { for: item }, item);

      const foundItems: number = filteredData.filter((e) => e[filter] === item).length;
      const totalItems: number = data.products.filter((e) => e[filter] === item).length;

      const countItemsElement: HTMLElement = this.createElement('span', 'checkbox-filter__count flex-fill');
